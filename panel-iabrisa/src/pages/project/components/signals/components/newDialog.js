...
export default function NewGameDialog(props) {
  const { onClose, project, setProject } = props;
  ...
  const onSubmit = async form => {
    ...
    try {
      ...
      if (!currencyPairs.length) {
        setError('currencyPairs', {
          type: 'required',
          message: 'Informe ao menos 01 moeda.'
        });
      ...
      const body = {
        ...
        games: {
          ...project.games,
          [Object.keys(project.games).length + 1]: form
        }
      };
    ...
    >
      <DialogTitle>
        <FlexBox justifyContent="space-between" alignItems="center">
          <Box>
            <H3 fontWeight={600}>Novo Padrão</H3>
          </Box>
            ...
            <Grid item xs={12}>
              ...
              <Autocomplete
                ...
                renderInput={params => (
                  <TextField
                    {...params}
                    error={!!errors.currencyPairs}
                    helperText={errors.currencyPairs?.message}
                  />
                )}
              />
            </Grid>
